...
> 本轮修正：留言板登录入口改为前台 `returnTo` OAuth 流程，公开 `comments.githubLoginConfigured` 布尔状态，未登录匿名留言后端返回 401；照片墙相册编辑弹窗支持 85vh 内部滚动；文章中枢链路恢复左右交替并贴靠中心线；搜索框去掉 `Search` 字样并增加深色搜索图标按钮。

### 当前前台体验补充

- 留言板：前台详情页使用 GitHub 登录后留言；未配置时显示访客友好提示，不暴露服务端 Secret 配置细节。
- 文章列表：支持矩阵网格和中枢链路两种显示模式，卡片标签靠近底部。
- 音乐：首页播放器和 `/music` 页面共享全局播放、音量和静音状态。
- 照片墙：后台相册编辑弹窗支持滚动管理组内照片。
