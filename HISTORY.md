...
- 新增 `backend/scripts/reset_admin.py`，用于忘记密码时重置管理员账号和密码哈希；新增 `backend/scripts/reset_install.py`，默认只移除 `.install.lock` 并保留业务数据，只有显式 `--wipe-data --yes` 才清空数据目录。
- `doctor.sh` 增加旧 `srblogs.service`、旧明文 `ADMIN_PASSWORD`、Nginx admin SPA 旧模板、安装状态与管理员凭据一致性检查。
- README、部署文档、部署脚本说明和环境变量模板同步改为安装向导/密码哈希口径，避免继续传播 `admin / change-me` 生产登录说明。

## 2026-05-22 - 后台内容管理信息结构精简

- 后台左侧导航收敛为“内容”和“设置”，移除审计日志、备份恢复入口展示；“内容管理”改名为“内容”，导航项不再显示说明小字。
- 评论从文章、图片、音乐子项中提升为与文章、图片、音乐、项目、友链、关于同级的独立入口。
- 内容管理页移除顶部 Hero 说明和文章统计卡片，文章管理直接从标题、筛选、刷新、新增、搜索和列表开始；图片、音乐、项目、友链、关于页同步去掉顶部说明区。
...
- 背景层按当前主题的 day/night token 读取 `slideshowEnabled`、`slideshowInterval` 和 `slideshowEffect`，支持分别控制白天和夜晚壁纸是否轮播、轮播间隔和切换动画。
- 昼夜切换时会立即触发一次背景图切换，并使用当前模式配置的淡入淡出/柔焦淡入/无动画效果。
- 后台“主题设置”的白天模式壁纸和夜晚模式壁纸子 Frame 增加轮播开关、轮播间隔和切换动画设置；保存后通过公开设置下发给前台。
- 文档同步记录：白天模式不使用背景蒙层，壁纸轮播配置属于公开视觉配置，不包含任何 Secret。

## 2026-05-14 - 壁纸设置生效与后台旧入口清理

- 修复前台主题 token 合并顺序：后台当前主题的白天/夜晚壁纸组现在优先覆盖主题包默认壁纸，避免保存新壁纸后仍显示旧背景。
- 后台保存主题交互设置时同步更新当前 active theme package 的 day/night `bgImages` 与 `activeBgIndex`，让导入导出和前台读取来源保持一致。
- 清理后台关于页编辑中的旧“兼容 Markdown”入口；关于页后台主流程只保留新版 Hero、关于我、GitHub 活动和联系表单结构化配置。
- 删除未接入主路由的旧版 SettingsPanels 组件，避免继续保留组件级低代码主题/布局导入导出等已弃用入口。
- 内容管理中的关于页说明同步改为结构化文案编辑，不再展示已弃用 Markdown 主流程。
- 待验证：刷新前台后确认白天/夜晚壁纸切换读取后台新配置；旧 `about.md` 文件保留为历史数据但不再出现在后台主流程。

## 2026-05-09 - 后台管理台主流程重构

- 按当前后台收口方向重构管理台信息架构，左侧主导航只保留“内容管理、设置、审计日志、备份恢复”四类入口，旧后台路径继续通过重定向兼容。
- 内容管理整合为文章、图片、音乐、项目、友链、关于及对应留言管理；文章页内用“正经 / 杂谈”开关切换，不再暴露低代码页面布局编辑入口。
- 写作编辑器改为平面化三栏工作流：基础信息、封面与素材、Markdown 正文；支持 `.md` 文件导入编辑区、正文图片上传插入、封面图上传和简介字数提示。
- 结构化内容管理器改为简洁列表 + 弹窗表单，保留相册组预览、批量上传、排序、设封面、音乐歌词上传、项目/友链链接与封面等能力。
- 设置页收敛为站点信息、我的信息、主题交互与留言授权四个 Frame；后台整体继续保持黑白灰平面工具风格，前台红白黑固定布局不受影响。
