...
- [ ] Admin credentials are initialized through `/install` or reset with `backend/scripts/reset_admin.py`.
- [ ] `ADMIN_PASSWORD_HASH` is present; legacy `ADMIN_PASSWORD` is absent or intentionally kept only for compatibility.
- [ ] `JWT_SECRET` is generated by the installer or changed to a long random value.
...
- [ ] `frontend/dist` does not contain admin credentials, `JWT_SECRET`, AI Key, OSS Key, or OAuth Secret.
- [ ] `admin/dist` does not contain admin credentials, `JWT_SECRET`, AI Key, OSS Key, or OAuth Secret.
