...
- 前台页面左右边距只能作用在统一主内容容器 `.site-page-container` 的 `padding-left` / `padding-right` 上。
...
- 音量浮层只允许出现一个垂直滑杆；如果使用 `input[type=range]`，必须通过自定义 track/thumb 收敛为单轨，不得再叠加额外 div 轨道。
- 音量滑杆白天/夜间都使用主题 token，轨道底色为灰阶，填充和 thumb 使用红色 accent。

## 2026-05-05 主题边距、背景组与播放器细节

- 前台内容区左右边距由主题配置 `themeConfig.layout.pagePadding` 控制，桌面、平板、移动端分别使用独立数值；页面容器应读取统一 CSS 变量，不要在单页硬编码。
