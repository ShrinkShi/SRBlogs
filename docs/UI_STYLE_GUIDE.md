...
- 壁纸是前台背景的主要视觉来源，昼夜蒙层只负责统一色调。
- 当前红白黑玻璃主题要求：日间模式不叠加背景蒙层，即 `overlayOpacity=0`；夜间模式可保留黑色蒙层以保证文字可读。
- day/night 壁纸轮播可分别配置开关、间隔和切换动画；昼夜切换时应立即触发一次壁纸淡入淡出切换，让模式切换有明确视觉反馈。
...
- 一级导航仅保留：内容、设置。审计日志和备份恢复作为兼容/排障页面保留，不再出现在日常侧栏导航。
