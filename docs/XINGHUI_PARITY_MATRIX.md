...
2026-05-03 前台评论入口修正与列表页统一改版轮后：

- `P0`：约 100%。内容读取/写入、登录鉴权、公开草稿保护、评论、媒体管理、设置边界、安全写入、审计日志、备份恢复、构建运行和 Secret 扫描均已通过验证；真实服务器部署实操单独标记为待执行。
- `P1`：约 98%。搜索、标签、归档、SEO/RSS/Sitemap、后台写作、草稿、pendingOperations 第一阶段、设置中心表单化、错误状态、操作反馈、真实使用手册和发布准备均已收口；GitHub OAuth 评论入口已在前台，真实授权仍需配置后人工验收。
- `P2`：约 90%。已完成铺满式顶部导航、滚动隐藏/显示、首页“名片 + 紧凑音乐播放器 + 歌词区 + 不对称内容模块 + 底部状态区”、主题 token、后台控制台视觉、Toast、全局音乐、相册组编辑、Markdown 拖动分栏、文章/杂谈三列图片卡和音乐页双栏布局；仍需浏览器人工回归后继续推进。
...
| 文章列表 | 文章卡片、标签、摘要、封面、列表浏览 | Posts 页面真实读取 `/api/posts`，已补加载、空、错误状态、封面兜底、标签、日期、摘要和详情跳转；公开列表默认不含草稿；P2 本轮改为居中标题、无外边框搜索、居中标签栏和三列图片卡片 | P0/P2 | 4 文章与评论轮 / 最终总回归轮 / P2 列表页统一改版轮 | 已完成 / P2 进行中 | P0 公开列表隐藏草稿、发布后可见、撤回/删除后消失已通过；P2 三列图片卡和 390px 仍需浏览器人工确认 |
...
| 评论 | Gitalk 风格评论，Issues/OAuth 配置 | 评论展示、评论索引、后台评论管理、删除备份和前后台同步已通过；新评论已改为 GitHub 登录后提交，入口在前台文章详情，未登录不显示输入框，未配置 OAuth 显示联系管理员提示；匿名评论后端返回 401 | P0/P1 | 4 文章与评论轮 / GitHub 评论收口轮 | 已完成 / GitHub OAuth 实测待执行 | 旧评论继续展示；GitHub OAuth Secret 不进前端；真实 GitHub 授权回跳需配置 Client ID/Secret 后人工验收 |
...
| P2 视觉增强 | 樱花、萤火、弹幕、动态背景细节、悬浮音乐细节、卡片动效和主题装饰 | 已有基础毛玻璃、背景和播放器氛围；已完成统一 token、卡片/按钮/标签动效、文章阅读层次、后台控制台样式、氛围开关、铺满式顶栏、首页结构追平、昼夜模式卡片、设置中心主题 token、三列列表页和音乐页双栏布局 | P2 | P2 视觉统一与氛围增强轮 / P2 首页结构追平轮 / P2 列表页统一改版轮 | 进行中 | 当前 P2 约 90%；需继续完成浏览器人工回归，确认首页结构、文章/杂谈/音乐页、390px、关闭动效和后台设置页不影响 P0/P1 |
