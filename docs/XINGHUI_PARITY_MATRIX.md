...
- 工具箱细节：进行中。已实现更不透明的设置/搜索弹窗、小浮窗计算器、交互点击特效和游客设置联动；仍需浏览器人工验收。
- 后台信息架构：进行中。已调整为页面编辑、内容管理、评论管理、后台设置、日志备份五个一级入口；旧路由保持兼容；仍需浏览器人工验收。
- 页面编辑第一阶段：进行中。已提供页面文案编辑、预览区拖拽/缩放雏形、保存 `pageLayouts` 和恢复默认布局；不标记完成，等待人工验证。
- 操作暂存区：进行中。默认完全隐藏，按需打开右侧抽屉；仍需人工确认不影响写作暂存流程。

# XinghuisamaBlogs Parity Matrix

## 2026-05-03 前台导航、文章整合、图片双模式与工具箱状态

- 顶部导航：进行中，待浏览器人工验收。目标导航为：首页、文章、图片、音乐、项目、友链、关于；搜索、杂谈、归档不再作为顶部入口。
- 文章整合页：进行中，待浏览器人工验收。`/posts` 现在承载“正经 / 杂谈”切换，两个板块均支持矩阵网格和中枢链路。
- 杂谈兼容：进行中，待浏览器人工验收。旧 `/chatters` 保持兼容并进入 `/posts?section=chatters`，杂谈详情路由继续保留。
- 图片页：进行中，待浏览器人工验收。`/photowall` 继续作为图片页使用，支持矩阵网格和中枢链路，相册弹窗与留言能力不应回退。
- 全局工具箱：进行中，待浏览器人工验收。左下角悬浮球提供计算器、全局搜索、游客设置；游客设置写入 localStorage，不涉及后端 Secret。
- 旧入口清理：进行中，待浏览器人工验收。首页右侧主题、背景、氛围、弹幕、点击音效旧按钮已从根应用移除，能力迁移到工具箱。
- 状态说明：本轮不再使用 P0/P1/P2 百分比汇报；未经过浏览器人工确认的打磨项不得标记为已完成。

## 2026-05-03 多平台留言登录专项修复状态

- 当前进度估算：P0 100%，P1 98%，P2 96%。
- 本轮专项修复 `GET /api/settings/public` 的留言 provider 状态结构，新增 `clientIdConfigured/appIdConfigured/secretConfigured` 布尔字段。
- GitHub 与 QQ 配置状态独立判断：GitHub `configured=true` 时前台必须显示 GitHub 登录按钮；QQ `configured=false` 只禁用或提示 QQ，不影响 GitHub。
- `/api/auth/github/login` 与 `/api/auth/qq/login` 均必须真实存在；未配置时可以返回中文友好错误，但不得返回 404 Not Found。
- 状态：多平台留言链路保持 `进行中/待真实 OAuth 浏览器验收`；API 与构建验证通过后，等待用户确认浏览器端登录按钮和跳转行为。
...
- 状态要求：P0 继续保持已完成；P1 中 GitHub OAuth 真实授权回跳仍需站点配置后人工验收；P2 继续进行中，等待浏览器确认视觉细节，不因构建通过直接标记整体完成。
## 2026-05-03 P1/P2 混合收口补修状态

- 当前进度估算：P0 100%，P1 约 99%，P2 约 88%。
- 本轮补修：Toast 移除 `.glass` 依赖并强制 fixed 顶层显示；GitHub 评论入口确认在前台文章详情，匿名评论后端 401；后台评论设置隐藏旧本地匿名/邮箱配置；相册编辑支持预览、增加、删除、拖动排序和设置封面；首页音乐播放器紧凑化并与名片高度接近；歌词卡片高度减半；名片统计和联系方式 hover/tooltip 增强。
- 仍需人工确认：真实 GitHub OAuth 授权回跳、Toast 在浏览器顶部显示、相册拖拽排序、首页播放器高度和 390px 回归。

## 2026-05-03 P1/P2 混合收口状态

- 当前进度估算：P0 100%，P1 约 98%，P2 约 86%。
- 本轮已实现：顶层 Toast、固定全高后台侧栏、设置中心子分区并入左侧层级、Markdown 可拖动分栏、自定义字体颜色 popover、点击音效公开配置、全局音乐播放、歌词上传、GitHub 登录评论基础链路、照片墙相册组、名片无边框化、轮播 hover 预选切换、移除首页播放器歌单按钮。
- P1 仍保留“进行中”尾项：GitHub OAuth 真实授权需要配置 Client ID/Secret 后浏览器人工验收；当前已验证未配置提示和匿名评论 401。
- P2 仍为“进行中”：首页/后台视觉与交互需要用户浏览器人工确认，不标记为全部完成。

## 2026-05-03 P2 交互细节与后台弹窗化收口状态

- 当前进度估算：P0 100%，P1 100%，P2 约 84%。
- 本轮已实现：全局顶层 Toast、后台设置页轻量顶部标签、主题 token 视觉优化、结构化内容新增/编辑弹窗、Markdown 近全屏弹窗、Markdown 快捷工具栏、字体颜色插入、上传大小按图片/音频/视频分档、点击音效开关、移除鼠标跟随大光晕和右下角冗余音乐播放器。
- P0/P1 保持完成状态；本轮没有新增业务模块，也没有修改核心内容数据结构。
- P2 仍为“进行中”：需要用户浏览器人工确认 Toast 位置、弹窗关闭行为、90MB 音频上传、Markdown 工具栏、点击音效、移动端和后台主要页面未回退。

## 2026-05-03 P2 第三阶段：轮播追平与后台结构修复状态

- 当前进度估算：P0 100%，P1 100%，P2 约 80%。
- 本轮已修复 P1 回归：`/admin/editor` 与 `/admin/about` 改为静态导入关键 Markdown 页面，避免开发环境动态 chunk 失效导致页面打不开。
- 首页 P2 本轮已实现：目标不对称轮播布局、最新文章轮播、照片墙轮播、最新更新轮播、独立昼夜切换卡片、图片铺满与图内文字/分页点、无图组件 hover 缩放、歌词单行自适应、绿色复制提示、前台导航移除后台入口。
- 后台 P2 本轮已实现：垂直父子目录导航、暂存区完全隐藏/恢复、中间主操作区扩宽、主题 token 中文标签与颜色选择器、本地图片/音频/视频上传入口增强。
- 首页、主题配置、后台导航和本地资源上传仍保持“P2 进行中”：需要用户浏览器人工确认 1440/1280/1024/768/390 宽度、轮播切换、hover、复制提示、上传和后台主要页面未回退。

## 2026-05-03 P2 首页轮播与播放器收口状态

- 当前进度估算：P0 100%，P1 100%，P2 约 72%。
- 首页 P2 本轮已实现：三组轮播卡片、最新文章轮播、最新更新内容轮播、主题模式轮播、唱片式首页播放器、居中歌词区、新名片结构、顶部导航慢动画、旧首页板块移除、hover 规则修正。
- 首页 P2 状态仍为“进行中”：需要用户浏览器人工确认 1440/1280/1024/768/390 宽度、轮播切换、唱片播放器播放/无 URL 兜底、复制提示、顶部导航动画和 hover 动效。
- P0/P1 已完成项不因本轮 P2 调整降级；本轮未修改后端接口和数据结构。

差距等级定义：

- `P0`：核心可用闭环，包括内容读取、内容写入、登录鉴权、设置读取、基础评论、构建运行、安全写文件。
- `P1`：关键体验，包括编辑器预览、草稿、暂存队列、搜索/标签、响应式、后台表单化设置、错误状态。
- `P2`：约 93%。首页、播放器、留言板、卡片、标题轻量化与中枢链路细节持续推进，仍需浏览器人工回归确认。

状态定义：`未开始`、`进行中`、`已完成`、`延期/未验收`、`部署实操待执行`、`冻结/未开始系统化增强`。只有满足本轮 Definition of Done 后才能标记为 `已完成`。

## 当前进度估算

2026-05-03 留言体系收口与列表页细节统一轮后：

- `P0`：约 100%。内容读取/写入、登录鉴权、公开草稿保护、评论、媒体管理、设置边界、安全写入、审计日志、备份恢复、构建运行和 Secret 扫描均已通过验证；真实服务器部署实操单独标记为待执行。
- `P1`：约 98%。核心体验已收口；GitHub OAuth 留言真实授权回跳仍需配置 Client ID/Secret 后人工验收。
- `P2`：约 93%。首页、播放器、留言板、卡片、标题轻量化与中枢链路细节持续推进，仍需浏览器人工回归确认。
...
| 文章列表 | 文章卡片、标签、摘要、封面、列表浏览 | Posts 页面真实读取 `/api/posts`，已补加载、空、错误状态、封面兜底、标签、日期、摘要和详情跳转；公开列表默认不含草稿；P2 已支持“矩阵网格 / 中枢链路”双模式、轻量搜索栏、居中标签和标签沉底 | P0/P2 | 4 文章与评论轮 / 最终总回归轮 / P2 列表页统一改版轮 | 已完成 / P2 进行中 | P0 公开列表隐藏草稿、发布后可见、撤回/删除后消失已通过；P2 双模式和 390px 仍需浏览器人工确认 |
...
| 杂谈 | 云端杂谈/碎片化文章卡片 | Chatter 页面真实读取 `/api/chatters`，公开路由统一为 `/chatters` 和 `/chatters/:slug`，旧 `/chatter` 保留重定向；已补加载、空、错误状态；P2 本轮改为居中标题、搜索栏和三列图片卡片 | P1/P2 | 6 媒体互动轮 / P2 列表页统一改版轮 | 进行中 | P1 读取与路由已可用；P2 三列卡片、移动端和详情页视觉仍需浏览器人工回归 |
...
| 照片墙 | 图片墙、封面、图集浏览 | 前台 Photowall 动态读取 `/api/photos`，已有加载/空/错误状态、懒加载、点击放大预览、标题/描述/日期/标签展示；后台照片管理主流程已表单化；P2 已扩展相册组预览、增加、删除、排序和设置封面能力 | P1/P2 | 6 媒体互动轮 / P2 相册组增强轮 | 已完成 / P2 进行中 | P1 照片墙和后台基础管理已通过人工验收；相册组内照片增删拖拽排序仍需用户浏览器确认 |
| 音乐 | 网易云 ID 配置和悬浮/页面播放器 | 前台 Music 动态读取 `/api/music`；后台歌单管理已表单化；P2 已实现全局播放状态、歌词上传/显示、首页紧凑唱片播放器和 `/music` 左播放器右歌词/歌单双页签布局 | P1/P2 | 6 媒体互动轮 / P2 音乐体验增强轮 | 已完成 / P2 进行中 | P1 音乐播放和后台管理已通过人工验收；P2 全局播放、歌词联动和音乐页双栏仍需浏览器人工确认 |
...
| 留言板 | GitHub 登录留言、旧评论展示、后台管理 | 评论展示、评论索引、后台评论管理、删除备份和前后台同步已通过；前台已统一为“留言板”，未登录显示 GitHub 登录按钮，未配置 OAuth 使用访客视角提示，匿名提交后端返回 401 | P0/P1/P2 | 4 文章与评论轮 / GitHub 留言收口轮 | 已完成 / GitHub OAuth 实测待执行 / P2 进行中 | 旧评论继续展示；GitHub OAuth Secret 不进前端；真实 GitHub 授权回跳和留言板视觉需浏览器人工验收 |
