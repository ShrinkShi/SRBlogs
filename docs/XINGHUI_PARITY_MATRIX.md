...
- `P1`：约 98%。搜索、标签、归档、SEO/RSS/Sitemap、后台写作、草稿、pendingOperations 第一阶段、设置中心表单化、错误状态、操作反馈、真实使用手册和发布准备均已收口；GitHub OAuth 留言真实授权仍需配置后人工验收。
- `P2`：约 92%。已完成铺满式顶部导航、滚动隐藏/显示、首页“名片 + 紧凑音乐播放器 + 歌词区 + 不对称内容模块 + 底部状态区”、主题 token、后台控制台视觉、Toast、全局音乐、相册组编辑、Markdown 拖动分栏、文章双模式、轻量搜索栏和留言板统一样式；仍需浏览器人工回归后继续推进。
