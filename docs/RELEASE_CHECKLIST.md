...
- [ ] 顶部导航为铺满式顶栏，滚动下隐藏、上滚显示、鼠标移到顶部显示。
- [ ] 首页结构为名片 + 音乐播放器、歌词区、不对称内容模块、底部状态区。
- [ ] 最新更新内容模块可用，且不包含草稿。
- [ ] 昼夜模式 token 体系可用，首页模式卡可切换日间/夜间。
- [ ] 后台设置中心可配置核心主题 token、字体族和字号档位。
- [ ] 前台视觉 token 使用统一类：`sr-card`、`sr-card-hover`、`sr-chip`、`sr-button-primary`、`sr-button-ghost`。
- [ ] 首页不出现右边距异常、组件裁切或最新文章撑宽。
- [ ] 文章详情阅读区不被背景、播放器、控制按钮或 sticky 导航遮挡。
...
- [ ] P2 视觉增强：按 `docs/UI_STYLE_GUIDE.md` 继续做系统化回归，不引入大型动画库、不做 3D、不破坏 P0/P1。
...
- [ ] 前台和后台主路径均完成快速回归，不白屏。
- [ ] `docs/USER_GUIDE.md` 已覆盖日常使用路径，README 已提供入口。
- [ ] `git status --short` 不包含未跟踪运行时备份、临时评论、上传缓存或 `tsbuildinfo`。
