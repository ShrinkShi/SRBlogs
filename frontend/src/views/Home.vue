...
<template>
  <section class="grid min-w-0 max-w-full gap-6 md:gap-8">
    <div class="home-hero-grid">
      <GlassCard class="sr-hero-panel min-w-0">
        ...
        <div class="mt-8 flex flex-wrap gap-3">
          <RouterLink to="/posts" class="sr-button-primary px-5 py-3">进入文章宇宙</RouterLink>
          <RouterLink to="/search" class="sr-button-ghost px-5 py-3">搜索全站</RouterLink>
          <RouterLink to="/archive" class="sr-button-ghost px-5 py-3">查看归档</RouterLink>
          <RouterLink to="/music" class="sr-button-ghost px-5 py-3 text-fuchsia-100/80">云音乐挂件</RouterLink>
        </div>
        <div class="mt-8 grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div class="sr-card-hover min-w-0 rounded-3xl border border-white/10 bg-white/[0.08] p-4"><p class="text-xs uppercase tracking-[.24em] text-white/38">frontend</p><b class="mt-2 block break-words text-white">Vue3 SPA</b></div>
          <div class="sr-card-hover min-w-0 rounded-3xl border border-white/10 bg-white/[0.08] p-4"><p class="text-xs uppercase tracking-[.24em] text-white/38">backend</p><b class="mt-2 block break-words text-white">FastAPI API</b></div>
          <div class="sr-card-hover min-w-0 rounded-3xl border border-white/10 bg-white/[0.08] p-4"><p class="text-xs uppercase tracking-[.24em] text-white/38">content</p><b class="mt-2 block break-words text-white">Markdown + JSON</b></div>
        </div>
      </GlassCard>
    ...
    <GlassCard>
      ...
      <div class="mt-5 grid gap-3 md:grid-cols-3">
        <RouterLink v-for="item in posts.slice(0, 3)" :key="item.slug" :to="`/posts/${item.slug}`" class="sr-card-hover min-w-0 rounded-2xl border border-white/10 bg-white/[0.06] p-4 hover:bg-white/[0.1]">
