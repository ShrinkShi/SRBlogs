...
<template>
  <section>
    <div class="mb-4 flex items-end justify-between gap-3">
      <div><p class="text-xs font-bold uppercase tracking-[.3em] text-cyan-100/45">latest posts</p><h2 class="mt-1 text-2xl font-black text-white">最新文章</h2></div>
      <RouterLink to="/posts" class="text-sm text-cyan-100/70 hover:text-cyan-100">查看全部</RouterLink>
    </div>
    <div class="no-scrollbar flex gap-4 overflow-x-auto pb-2">
      <RouterLink v-for="item in items" :key="item.slug" :to="`/posts/${item.slug}`" class="glass glass-hover group min-w-[280px] overflow-hidden rounded-[30px] md:min-w-[360px]">
        ...
        <div class="relative z-[1] p-5">
          ...
          <div class="mt-4 flex flex-wrap gap-2"><span v-for="tag in item.meta.tags?.slice(0,3)" :key="tag" class="rounded-full border border-white/10 px-2 py-1 text-[11px] text-white/50">#{{ tag }}</span></div>
        </div>
      </RouterLink>
    </div>
  </section>
