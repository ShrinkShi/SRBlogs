...
<template>
  <header class="fixed left-0 right-0 top-0 z-40 py-3 md:py-4">
    <nav class="glass sr-page-shell flex min-w-0 items-center justify-between rounded-[28px] px-4 py-3 md:px-5">
      ...
      <button class="relative z-[2] shrink-0 rounded-2xl border border-white/15 px-3 py-2 text-sm text-white/80 lg:hidden" @click="open = !open">菜单</button>
    ...
    <div v-if="open" class="glass sr-page-shell mt-2 grid grid-cols-2 gap-2 rounded-[28px] p-3 sm:grid-cols-3 lg:hidden">
