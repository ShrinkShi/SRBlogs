...
后台“设置 > 版本更新”可以读取当前本地版本，并手动检查 `ShrinkShi/SRBlogs` 的 GitHub Releases。发现新版本后，管理员可以选择忽略该版本，或在服务端已配置更新脚本时触发“一键更新”。

一键更新默认关闭，只能由后端环境变量启用。前端和后台页面不会提交、拼接或回显 shell 命令：

```env
SRBLOGS_UPDATE_REPO=ShrinkShi/SRBlogs
SRBLOGS_UPDATE_ENABLED=false
SRBLOGS_UPDATE_COMMAND=
```

生产环境启用前，应把 `SRBLOGS_UPDATE_COMMAND` 指向已审查、可回滚的部署脚本，并确认脚本不会把 OAuth Secret、SMTP Secret、管理员 token 或其他敏感信息写入更新日志。
